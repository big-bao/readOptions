...
public class OfFlowUtils {

    private static final Logger LOG = LoggerFactory.getLogger(OfFlowUtils.class);
    ...
    public static FlowBuilder getPipelineFlow(short table, short gotoTable) {
        ...
        return initFlowBuilder(flowBuilder, flowName, table).setPriority(0);
    }
     ...
     *            the match builder
     * @param tableNo
     *            the table no to which flow needs to be inserted.
     * @return the created flow builder.
     */
    public static FlowBuilder createFlowBuilder(String flowName, Integer priority, MatchBuilder matchBuilder,
            short tableNo) {
        FlowBuilder flowBuilder = new FlowBuilder();
    ...
    /**
     * Sets up common defaults for the given flow builder: a flow identifier and
     * key based on the given flow name, strict, no barrier, the given table
     * identifier, no hard timeout and no idle timeout.
     *
     * @param flowBuilder
     *            The flow builder.
     * @param flowName
    ...
    public static FlowBuilder initFlowBuilder(FlowBuilder flowBuilder, String flowName, short table) {
        ...
        flowBuilder.setId(flowId).setStrict(true).setBarrier(false).setTableId(table).setKey(new FlowKey(flowId))
                .setFlowName(flowName).setHardTimeout(0).setIdleTimeout(0);
        return flowBuilder;
    }
